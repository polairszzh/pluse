...
2. 运行执行层脚本：
   ...
   - 引用质量分层（可选）：`--mine-owned "<转载标识>"` 标记转载/自有渠道（命中记「转载」）；`--competitor "<竞品标识>"` 用于 lostprompt（竞品夺走）分析
3. 打开 `data/snapshots/` 下刚生成的 `track-*.md` 报告，核对各平台状态、「我的内容」列、趋势与行动清单。
...
3. 诚实标注数据来源：DeepSeek = 真实 API；Kimi/豆包/元宝 = 搜索引擎存在信号推断；`--mine` 的「我的内容」是子串匹配，结果为否 = 尚未被引用（如实报告，不夸大）。
