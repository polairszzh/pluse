...
Pulse 的目标是让 AI 准确理解你的价值，不是操控引用。五条不可越界的红线：

1. **不编造**——数据、来源、引用必须有出处；LLM 只改文风，不造事实。
2. **不批量灌水**——每篇输出都是「人写 + 人审」，Pulse 不提供一键批量生产。
3. **利益披露**——推荐课程/网站/商品必须声明利益关系，营销转化信号（扫码/私信/优惠码/必买等）会被直接拒绝生成。
4. **不碰 AI 呈现**——不做隐藏提示词、不针对 AI 检索漏洞动手脚。
5. **守平台规则**——不刷量、不诱导、不绕限流。

开源工具没有使用者准入，所以护栏内置在产品里：素材缺口清单、发布前人工检查、营销转化信号直接拒绝生成（exit 3）、数据断言多源交叉验证（无法核实标注不确定性、与来源冲突拒绝生成）、falsifiability check。
...
| `/pulse adapt --source <草稿.md>` | 本地草稿 → 知乎版 + AI 优化版（草稿四维打分 + 验证钩子）；小红书/视频版规划中 |
...
**多平台内容适配（Phase 3 · 开发中 MVP）** — 本地 Markdown 草稿 → 知乎版 + AI 优化版。AI 优化版：每个 H2 一个问答对、首段 130-170 字自包含答案，写给 AI 摘取；知乎版：结构化改写 + 适度扩写。草稿先做四维打分（AI 可引用性/内容质量/关键词覆盖/结构，互动维度标未发布），每个生成版本带 falsifiability check（发布后重跑 `track --mine` 验证）。生成 = 规则脚手架 + DeepSeek 改写（无密钥自动回退脚手架）。小红书/视频版规划中。
...
| **Phase 3** | `/pulse adapt` — 多平台内容适配（知乎 + AI 优化版 MVP） | 开发中 |
