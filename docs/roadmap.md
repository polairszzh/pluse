...
- **三级判定**：
  - 已核实（权威源一致）→ 放行；
  - 疑似冲突（来源间矛盾，且无权威源支持）→ **拒绝生成（exit 3，同营销信号机制）**，列出冲突证据与建议核验的官方渠道；
  - 无法核实（无可靠来源）→ 不阻断，但输出内容带显式标注「信息可能存在延迟或不确定性，建议通过官方渠道核验」，并进检查清单。
- **风险领域标记**：医学偏方、软件版本号、价格/计费政策等易传播错误信息的领域，即使无法核实也强制标注不确定性。
- **引用原则**：只做事实核查，不做「搬运工」——验证结论只用于放行/标注/拒绝，不把搜索结果搬进生成内容。
- **依赖与前置**：复用 `_scan_facts`（数字断言提取）与 Bing 搜索通道；建议先落地 track 置信度标签（Phase 4 第一批，稳定搜索验证基建、积累验证经验），再实现防火墙，降低「疑似冲突误杀」风险。
  **实现**（scripts/fact_checker.py + adapt 集成）：数字断言+上下文提取（跳过第一手经验）、Bing 多源交叉、权威度分级、否定信号优先判 conflict（exit 3）、无法核实进素材缺口（风险领域 high/普通 medium）、失败降级不阻断。真实验证：WorkBuddy 草稿「5000积分」被搜索证实放行，「100积分（价格/政策）」「10分钟/15分钟/8GB」标无法核实。
