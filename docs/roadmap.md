...
- **优先级**：P1。
  **实现**（2026-08-15，PR #20）：新增 `scripts/bottleneck.py`（八层映射为四类可诊断层：记忆/索引层、检索/选择层、引用层、风险/治理层）；CLI `bottleneck.py --query <话题> [--mine <标识>] [--index-status <indexed|not_indexed|unknown>]`；adapt 集成——score_draft 前自动诊断，manifest 输出 bottleneck 字段、CLI 显示「瓶颈定位/依据/方向」，诊断失败降级不阻断。
