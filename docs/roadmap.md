...
- 失败/风控（40362）自动降级到 API 摘要，报告标注「浏览器采集」与「摘要级」；只读 + 低频 + 请求间隔；隐藏自动化特征以读取公开页面（禁用自动化标记、移除 webdriver、UA 去 HeadlessChrome 标记——UA 为浏览器内置版本，不伪造特定版本/冒充用户）；批量监测不依赖此通道。
...
- **优先级**：P1。
  **实现**（2026-08-15，PR #20）：新增 `scripts/bottleneck_diag.py`（八层映射为四类可诊断层：记忆/索引层、检索/选择层、引用层、风险/治理层；无有效历史时判 unknown，显式 index_status 优先）；CLI `bottleneck_diag.py --query <话题> [--mine <标识>] [--index-status <indexed|not_indexed|unknown>]`；adapt 集成——版本生成前自动诊断，manifest 输出 bottleneck 字段、CLI 显示「瓶颈定位/依据/方向」，诊断失败降级不阻断。
...
- **优先级**：P1。依赖：benchmark 数据集扩充。
  **实现**（2026-08-15，PR #21）：新增 score_evidence_citation（权威引语《》/直接引语、统计数据、来源标记、权威域名 gov.cn/edu.cn/官方）；audit_article 六维权重校准——AI可引用性 25 + 证据引用 18（合计 43%）+ 内容质量 22 + 关键词 17 + 结构 9 + 互动 9；AuditScores/render_json 补 evidence_citation。benchmark 30 篇重打分：证据均分 52（ai_search 69 > 知乎 46 > 个人品牌 42），区分度合理。
