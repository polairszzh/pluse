...
class TestVerifyFact:
    ...
    def test_wu_renhe_reject(self, monkeypatch):
        # 「无任何 5000 积分」是明确否定（无 + 任何 + 数字）
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "官方无任何 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] == "conflict"

    def test_mei_you_zhengshi_not_conflict(self, monkeypatch):
        # 「没有证实」是中性（未证实 ≠ 否定/属实），不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "官方没有证实 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_you_yaoyan_cheng_not_conflict(self, monkeypatch):
        # 「有谣言称」是传闻（无定论），不判 conflict
        html = bing_html([
            ("来源", "https://baike.baidu.com/item/x", "有谣言称 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_wei_fa_bu_not_conflict(self, monkeypatch):
        # 「未发布」是中性（与「尚未发布」一致），不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "官方未发布 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_wei_fa_fang_reject(self, monkeypatch):
        # 「未发放」仍是明确否定（与未发布区分）
        html = bing_html([
    ...
    def test_mei_you_menkan_not_conflict(self, monkeypatch):
        # 「没有门槛领」是肯定（无门槛可领），不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "没有门槛领 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_bu_shi_meigeren_not_conflict(self, monkeypatch):
        # 「不是每个人都有」是限定表述，不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "不是每个人都有 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_cheng_qing_shi_yaoyan_reject(self, monkeypatch):
        # 「已澄清 5000 积分是谣言」（无冒号/为/是紧跟）→ 否定 → conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "官方已澄清 5000 积分是谣言"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
    ...
    def test_wei_zhao_dao_not_confirmed(self, monkeypatch):
        # 「未找到」是中性（未找到 ≠ 断言存在），不判 confirmed
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "官方未找到 5000 积分活动"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "confirmed"

    def test_bing_fei_meigeren_not_conflict(self, monkeypatch):
        # 「并非每个人」是限定表述，不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "并非每个人都能领 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"

    def test_bu_shi_meiwei_not_conflict(self, monkeypatch):
        # 「不是每位用户」是限定表述，不判 conflict
        html = bing_html([
            ("官方说明", "https://baike.baidu.com/item/x", "不是每位用户都有 5000 积分"),
        ])
        monkeypatch.setattr(requests, "get", lambda *a, **kw: FakeResponse(text=html))
        r = verify_fact("workbuddy", "5000积分")
        assert r["status"] != "conflict"
