...
1. To check whether a specific article is indexed by search engines (B5 国内收录), run `python scripts/search_ai.py --index-check <文章URL>` (mutually exclusive with `--query` and other track args like `--samples/--db/--output/--mine`; passing them together exits with code 2): outputs Bing/百度 收录状态 and saves a JSON snapshot under `data/snapshots/index-check-*.json`. Baidu may report 探测失败 due to anti-scraping (honest limitation); Sogou/Bocha pending.
