...
Before rewriting, `adapt` runs an A2 bottleneck diagnosis (scripts/bottleneck_diag.py) against the topic's track history: it classifies why content isn't being cited — 记忆/索引层 (not indexed/never mentioned → fix indexing first, rewriting is ineffective), 检索/选择层 (mentioned but your content not selected → adapt content), 引用层 (already cited → keep), 风险/治理层 (cited with risks → govern). The diagnosis appears in the CLI output and manifest as `bottleneck`; standalone: `python scripts/bottleneck_diag.py --query <话题> [--mine <标识>] [--index-status indexed|not_indexed|unknown]`.
